    ...
    if($con==null){
        ...
        if($result->num_rows > 0){
            ...
            while($row = $result->fetch_assoc()){
                ...
                $cur_cell += 1;
                $card = "<td><div class='card_holder ".$row["card_rarity"]."'><a href='card.php?id=".$row["CAID"]."'><img alt='".$row["card_name"]."' src='/clash/images/".$row["card_image"]."'><div class='name_holder'><h2>".$row["card_name"]."</h2></div></a></div></td>";
                $card_table.=$card;
                if($cur_cell == 4){
                    $card_table .= "</tr>";
                    $cur_cell = 0;
